...
# ---- IO Settings ----
OUTPUT_DIR_BASE = r"Result\BatchRun\4\single_r50km"
OUT_ENABLE = True
OUT_MAKE_DIR = True
OUT_PRINT = True
OUT_CSV_RUNLOG = True
...
def plot_and_save_cdf(storage_dict, xlabel, filename_suffix, xlim=None):
    ...
    if len(storage_dict) == 0:
        ...
        for (label, data), color in zip(storage_dict.items(), colors):
            ...
            plt.plot(sorted_data, yvals, label=f"R=50km", color=color, linewidth=2)
            plotted_any = True
    ...
    if len(storage_dict) == 0:
        ...
        for (label, data), color in zip(storage_dict.items(), colors):
            ...
            try:
                ...
                plt.plot(xs, density(xs), label=f"R=50km", color=color, linewidth=2)
...
if __name__ == "__main__":
    ...
    scenarios = [
        (5.0,  os.path.join(base_path, "HAPS_beamlist_single_r50km.csv")),
