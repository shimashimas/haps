...
HAPS_ALT_KM   = 20.0
CARRIER_FREQ  = 2.0e9
REPEATED_BEAM = 1     # マルチビームなら3, シングルなら1
...
# ---- IO Settings ----
OUTPUT_DIR_BASE = r"Result\BatchRun\4\disaster\single"
OUT_ENABLE = True
OUT_MAKE_DIR = True
OUT_PRINT = True
OUT_CSV_RUNLOG = True
...
def plot_and_save_cdf(storage_dict, xlabel, filename_suffix, xlim=None):
    ...
    if len(storage_dict) == 0:
        ...
        for (label, data), color in zip(storage_dict.items(), colors):
            ...
            plt.plot(sorted_data, yvals, label=f"R=18km", color=color, linewidth=2)
            plotted_any = True
    ...
    if len(storage_dict) == 0:
        ...
        for (label, data), color in zip(storage_dict.items(), colors):
            ...
            try:
                ...
                plt.plot(xs, density(xs), label=f"R=18km", color=color, linewidth=2)
...
if __name__ == "__main__":
    ...
    storage_sinr = {}
    base_path = r"database\beamlist_single"
    scenarios = [
        (1.0,  os.path.join(base_path, "HAPS_beamlist_single_r18km.csv")),
        #(1.0,  os.path.join(base_path, "HAPS_beamlist_r1km_p1.2_disaster_cx21_cy25_r18.csv")),
    ]
